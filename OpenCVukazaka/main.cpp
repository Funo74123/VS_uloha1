...
CvCapture *input_video = cvCaptureFromFile("D:/temp3.mp4");
if (input_video == NULL)
{
...
CvPoint2D32f frame1_features[1500];
...
*/
float optical_flow_feature_error[1500];
...
optical_flow_termination_criteria, 2 );
...
cv::Mat image=cv::cvarrToMat(frame1);
Mat gray;
  ...
  //  for( size_t i = 0; i < circles.size(); i++ )
   // {
     //    Point center(cvRound(circles[i][0]), cvRound(circles[i][1]));
		 ...
		 double dist=781*50/radius;
		printf("Closest distance: %f mm",dist);
         // draw the circle center
		 //cvCircle( frame1, center, 3, Scalar(0,255,0), -1, 8, 0 );
         // draw the circle outline
         //cvCircle( frame1, center, radius, Scalar(0,0,255), 3, 8, 0 );
 //   }
...
q.x = (int) frame2_features[i].x;
q.y = (int) frame2_features[i].y;
int asd=sqrt(pow((double)q.x-double(p.x),2)+pow((double)q.y-double(p.y),2));
//if (asd<20){
//printf("Closest distance: %f",651.*5/asd);
...
if (current_frame >= number_of_frames - 1) {current_frame = number_of_frames - 2; return 0;}
}
}
